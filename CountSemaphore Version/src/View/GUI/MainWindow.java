...
public class MainWindow implements Initializable{
    ...
    Button runButton;
    @FXML
    TableView<Map.Entry<Integer, Pair<Integer,ArrayList<Integer>>>> semaphoreTable;
    @FXML
    TableColumn<Map.Entry<Integer, Pair<Integer,ArrayList<Integer>>>,String> indexCol;
    @FXML
    TableColumn<Map.Entry<Integer, Pair<Integer,ArrayList<Integer>>>,String> valueCol;
    @FXML
    TableColumn<Map.Entry<Integer, Pair<Integer,ArrayList<Integer>>>,String> threadsCol;
    ...
    public void initialize(URL location, ResourceBundle resources) {
        ...
        indexCol.setCellValueFactory(p->new SimpleStringProperty(p.getValue().getKey() + " "));
        valueCol.setCellValueFactory(p->new SimpleStringProperty(p.getValue().getValue().getKey() + " "));
        threadsCol.setCellValueFactory(p->new SimpleStringProperty(p.getValue().getValue().getValue() + " "));

        heapTableAddress.setCellValueFactory(p-> new SimpleIntegerProperty(p.getValue().getKey()).asObject());
        heapTableValue.setCellValueFactory(p->new SimpleStringProperty(p.getValue().getKey() + " "));
    ...
    private void setSemaphoreTable() {
        semaphoreTable.setItems(FXCollections.observableList(new ArrayList<>(myController.getRepository().getProgramList().get(0).getSemaphoreTable().getContent().entrySet())));
